...
void Console::Draw(const char* title, bool* p_open) {
    ...
    ImGui::SameLine();
    Filter.Draw(" Filter (\"incl,-excl\") (\"error\")" "(\"message\")", 180);
    ImGui::Separator();
    ...
    if (ImGui::BeginChild("ScrollingRegion", ImVec2(0, -footer_height_to_reserve), ImGuiChildFlags_NavFlattened, ImGuiWindowFlags_HorizontalScrollbar))
    {
        ...
        for (const char* item : Items)
        {
            ...
            else if (strstr(item, "[message]")) { color = ImVec4(0.4f, 1.0f, 1.0f, 1.0f); has_color = true; }
            else if (strncmp(item, "> ", 2) == 0) { color = ImVec4(1.0f, 1.0f, 1.0f, 1.0f); has_color = true; }
            if (has_color)
